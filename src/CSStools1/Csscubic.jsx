...
export default function CubicBezierGenerator() {
  ...
  useEffect(() => {
    if (animRef.current) {
      ...
      if (playing) {
        animRef.current.style.animation = `moveDot ${duration}s cubic-bezier(${points.join(",")}) forwards`;
      }
    }
  ...
  return (
    <div className="min-h-screen max-w-4xl m-auto  px-4 py-8 font-sans">
      {/* Header */}
      <div className="flex items-center justify-center gap-2 mb-6">
        ...
        <h1 className="text-xl font-semibold text-gray-900">CSS Cubic Bezier Generator</h1>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6 flex flex-col gap-6 max-w-6xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="bg-[#f7f8fa] rounded-lg border px-4 py-3 flex flex-col items-center w-full max-w-md mx-auto">
            <svg width={width} height={height} className="mb-2">
              <line x1={pad} y1={height - pad} x2={width - pad} y2={height - pad} stroke="#d1d5db" strokeWidth="2" />
              <line x1={pad} y1={height - pad} x2={pad} y2={pad} stroke="#d1d5db" strokeWidth="2" />
              <path d={`M${sx},${sy} C${p1x},${p1y} ${p2x},${p2y} ${ex},${ey}`} stroke="#2563eb" strokeWidth="2" fill="none" />
              <line x1={sx} y1={sy} x2={p1x} y2={p1y} stroke="#a7f3d0" strokeDasharray="2,2" />
              <line x1={ex} y1={ey} x2={p2x} y2={p2y} stroke="#fecaca" strokeDasharray="2,2" />
              ...
              <text x={sx - 18} y={sy + 18} fontSize="12" fill="#555">(0,0)</text>
              <text x={ex + 6} y={ey - 8} fontSize="12" fill="#555">(1,1)</text>
              <text x={width / 2 - 18} y={height - 2} fontSize="13" fill="#888">Time</text>
              <text x={2} y={height / 2} fontSize="13" fill="#888" transform={`rotate(-90 10,${height / 2})`}>Progress</text>
            </svg>
            <div className="text-center mt-1">
              <div className="text-xs text-gray-500">Cubic Bezier Coordinates</div>
              <div className="font-mono text-lg font-bold">({points.map(formatNum).join(", ")})</div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-500 mb-1">Predefined Easing Functions</label>
                <select
                  className="w-full border border-gray-200 rounded px-2 py-1 bg-white text-gray-900"
                  value={preset}
              ...
              <div className="flex-1">
                <label className="block text-xs font-medium text-gray-500 mb-1">
                  Animation Duration:{" "}
                  <span className="font-semibold text-gray-700">{duration}s</span>
                </label>
                <input
                  type="range"
                  min="0.2"
              ...
              {[0, 2].map((i, idx) => (
                <div key={idx}>
                  <div className="flex items-center gap-1 mb-1">
                    <span className={`h-2 w-2 rounded-full ${i === 0 ? "bg-green-500" : "bg-red-500"}`} />
                    <span className={`text-xs font-semibold ${i === 0 ? "text-green-700" : "text-red-700"}`}>
                      Coordinates of P{i / 2 + 1} ({i === 0 ? "Green" : "Red"} Dot)
                    </span>
                  </div>
                  <label className="block text-xs text-gray-500 mb-0.5">
                    X{i / 2 + 1}: <span className="text-gray-700">{formatNum(points[i])}</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                  ...
                  <label className="block text-xs text-gray-500 mt-2 mb-0.5">
                    Y{i / 2 + 1}: <span className="text-gray-700">{formatNum(points[i + 1])}</span>
                  </label>
                  <input
                    type="range"
                    min="0"
        ...
        {/* Animation Preview */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Animation Preview</label>
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1 h-2 bg-gray-200 rounded overflow-hidden">
              <div
                ...
                style={{
                  animation: playing
                    ? `moveDot ${duration}s cubic-bezier(${points.join(",")}) forwards`
                    : "none",
                }}
              />
            </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              className="px-4 py-1.5 bg-gray-100 border rounded text-gray-700 hover:bg-gray-200 text-sm flex items-center gap-1"
              onClick={handleReset}
            >
            ...
            <button
              className="px-4 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm font-semibold flex items-center gap-1"
              onClick={handleCopy}
            >
              <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
